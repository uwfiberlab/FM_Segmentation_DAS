#model from (Shi et al., 2025), loss_fn function changed from MSEloss to cross entropy loss, also utilized datalabel class
...
from das_denoise_models import unet, datalabel
...
def train(configure_file='config.ini'):
    ...
    with h5py.File(datah5, 'r') as f:
        time_data = f['Acquisition']['Raw[0]']['RawData'][:1500, :1500]
    ...
    y = y.T[np.newaxis, :, :]  # Transpose and add a new axis for labels
    x = np.repeat(x, 5, axis=0)
    y = np.repeat(y, 5, axis=0)
    print(x.shape, y.shape, x.dtype, y.dtype) #batch 1st dimension, then 1500 by 1500

    training_data = datalabel(x[:4], y[:4])  # Use datalabel for training data
    validation_data = datalabel(x[4:], y[4:])  # Use datalabel for validation data
